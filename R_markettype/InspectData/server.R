...
shinyServer(function(input, output, session) {
    ...
    #current data row
  # input <- 1
  # mt_analysed <- macd_ai[input, 65]
  # ln_analysed <- macd_ai[input, ]
  
  
    mt_analysed <- reactive({ mt_analysed <- macd_ai[input$rows, 33]})
    ...
    output$AI_Data <- renderPlot({
        ...
        plot(x = 1:32, y = macd_ai[input$rows, 1:32],type = "l", main = mt_analysed())
        abline(h=0, col="blue")
